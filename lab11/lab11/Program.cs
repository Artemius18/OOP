...
namespace lab11
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            Assembly testedAssmbly = Assembly.LoadFrom(@"D:\BSTU\2course\1term\OOP\Lab_2\Lab_2\bin\Debug\Lab_2.exe");

            Type? testedClass = testedAssmbly.GetType("Lab_2+Product");
            ...
            using (StreamWriter sw = new StreamWriter("Reflection.txt", false, Encoding.Default))
            {
                ...
                Reflector.Invoke(testedClass,
                                 testUser,
                                 "MethFor11Lab",
                                 /* Reflector.GetParametersFromFile(testedClass, "MethFor11Lab", "Invoke.txt") *///ДАНИЛА, введи число
                                 Reflector.GetRandomParameters(testedClass, "MethFor11Lab"));
                sw.WriteLine("****************************************************************************");
            }
            ...
            testedClass = typeof(Int32);

            using (StreamWriter sw = new StreamWriter("Reflection_4.txt", false, Encoding.Default))
            {
                sw.WriteLine("-----определение имени сборки------");
                sw.WriteLine(Reflector.GetAssemblyName(testedClass));
                sw.WriteLine("-----наличие публичных конструкторов----- ");
                ...
                var testInt = Reflector.CreateObject(testedClass);
